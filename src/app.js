...
198.51.100.77 - - [05/Jun/2026:12:10:00 +0200] "GET /agb HTTP/1.1" 200 800 "-" "-"`;
      ...
      function preflightLogSample(text, options = {}) {
        ...
        const warnings = [];
        if (result.hostCounts.size > 1) warnings.push("Mehrere Hosts/Domains in der Stichprobe erkannt.");
        if (options.useXff && !result.xffUsed) warnings.push("X-Forwarded-For aktiviert, aber in der Stichprobe nicht plausibel gefunden.");
        if (recognitionRate < 0.95) warnings.push(`Nur ${percent(recognitionRate * 100)} der Datenzeilen in der Stichprobe erkannt.`);
        return {
        ...
        const evidence = {
          pageViews: {
            ...
            reason: originCacheRisk
              ? "Origin-Log hinter Proxy/CDN: Seitenaufrufe sind fuer Origin-Traffic gemessen, koennen aber CDN-Cache-Hits verfehlen."
              : (pageviewReliability === "limited" ? "Zu viele Datenzeilen wurden nicht sicher erkannt." : "Seitenaufrufe wurden direkt aus erkannten Logzeilen gezaehlt.")
          },
          visits: {
            ...
            reason: proxyKind && !config.useXff
              ? "Proxy/CDN-Muster ohne belastbare XFF-Auswertung: echte Besucherzahl ist mit diesen Daten nicht serioes bestimmbar."
              : "Besucher werden aus IP, User-Agent und 30-Minuten-Fenster geschaetzt."
          },
          conversions: {
          ...
          ga4: {
            ...
            reason: lastGa4Import.warning || (!overall ? "Keine belastbaren GA4-Seitenaufrufe eingetragen." : (hostReliability === "limited" ? "Mehrere Hosts ohne Hostfilter machen den GA4-Abgleich nicht sauber vergleichbar." : "GA4-Seitenaufrufe wurden mit normalisierten Serverpfaden verglichen."))
          },
          hostScope: {
            type: hosts.total ? "measured" : "not_available",
            reliability: hostReliability,
            canAnswer: hostReliability !== "limited",
            reason: hostReliability === "limited" ? "Mehrere Hosts/Domains erkannt; ohne Hostfilter koennen fremde Domains enthalten sein." : "Host-Scope wirkt eindeutig."
          },
          botAnomaly: {
            type: "estimated",
            reliability: botReliability,
            canAnswer: !agg.trackingCapped,
            reason: agg.trackingCapped ? "Tracking-Speicherlimit begrenzt Bot-/Anomalie-Erkennung." : (agg.suspiciousClients ? "Auffaellige Besucher-Schluessel mit vielen Aufrufen fast ohne Assets gefunden." : "Keine auffaellige Bot-/Monitoring-Konzentration erkannt.")
          }
        };
        return {
      ...
      function qualityReason(data, metric, hasGa4) {
        if (metric === "views") {
          ...
          if (data.evidence && data.evidence.pageViews && data.evidence.pageViews.type === "lower_bound") return `Nur Mindestwert fuer Origin-Traffic: ${recognized} erkannt, aber CDN-Cache-Hits koennen im Origin-Log fehlen.`;
        ...
        if (metric === "visits") {
          if (data.evidence && data.evidence.visits && data.evidence.visits.type === "not_determinable") return "Nicht serioes bestimmbar: Proxy-/CDN-Muster ohne belastbare XFF-Auswertung.";
      ...
      function render(data) {
        ...
        id("visits").textContent = data.evidence && data.evidence.visits && data.evidence.visits.type === "not_determinable"
          ? `Nicht serioes bestimmbar (technische Bandbreite ${format(data.visitorRange.low)}–${format(data.visitorRange.high)})`
          : data.visitorRange && data.visitorRange.low !== data.visitorRange.high
      ...
      function setVerdict(data, hasGa4, hasConv) {
        if (!hasGa4 && !hasConv) {
          ...
          const visitText = data.evidence && data.evidence.visits && data.evidence.visits.type === "not_determinable"
            ? "eine mit diesen Daten nicht serioes bestimmbare Besucherzahl"
