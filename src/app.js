      ...
      function ga4UrlViews() {
        ...
        if (lines.length && values.size === 0 && !lastGa4Import.warning) {
          lastGa4Import.warning = "Die GA4-Eingabe konnte nicht als Seitenaufrufe gelesen werden. Bitte Export mit Page path und Views/Page Views verwenden.";
        }
        return values;
      }
      ...
      function buildResult(agg, config) {
        ...
        if (overall && ga4Validation.unmatchedRows > 0 && ga4Validation.unmatchedShare >= 0.5) {
          addConflict(
            ...
            "high",
            `${format(ga4Validation.unmatchedRows)} GA4-Seiten haben keinen Treffer in der Server-Datei.`,
            "Prüfen, ob GA4-Zeitraum, Website und Seitenauswahl wirklich zur Server-Datei passen.",
            ["ga4_decision"]
          );
        }
        if (overall && ga4Validation.duplicateCount > 0) {
          addConflict(
            "ga4_duplicate_pages",
            "medium",
            "GA4 enthält dieselbe Seite mehrfach. ServerStory summiert diese Zeilen, aber der Export sollte geprüft werden.",
            "Doppelte GA4-Zeilen entfernen oder den Export nach Seite zusammenfassen.",
            ["ga4_decision"]
          );
        }
        ...
        const evidence = {
          ...
          ga4: {
            ...
            reason: lastGa4Import.warning || (!overall
              ? "Keine brauchbaren GA4-Seitenzahlen eingetragen."
              : (hostReliability === "limited"
                ? "Die Datei enthält mehrere Websites. Ohne Filter ist der GA4-Vergleich unsicher."
                : (ga4DecisionBlocked ? conflicts.find((conflict) => conflict.blocks.includes("ga4_decision")).text : "GA4 wurde mit denselben Seitenadressen aus der Server-Datei verglichen.")))
          },
          hostScope: {
          ...
          botAnomaly: {
            ...
            reason: agg.trackingCapped ? "Die Schutzgrenze fuer sehr grosse Dateien begrenzt Bot- und Auffaelligkeits-Hinweise." : (agg.suspiciousClients ? "Auffaellige Muster mit vielen Aufrufen fast ohne normale Seitenbestandteile gefunden." : "Keine starke Bot-/Monitoring-Auffaelligkeit sichtbar.")
          }
        };
        const claims = {
          pageViews: {
            ...
            forbiddenConclusions: exportCompleteness.reliability === "limited" || pageviewReliability === "limited"
          ...
          visits: {
            ...
            forbiddenConclusions: visitorReliability === "limited"
          ...
          ga4: {
            ...
            forbiddenConclusions: !(overall && !lastGa4Import.warning && hostReliability !== "limited" && pageviewReliability !== "limited" && exportCompleteness.reliability !== "limited" && !ga4DecisionBlocked)
              ...
              : ["GA4-Abweichung nicht ohne Zeitraum-, Website- und Cache-Prüfung als Tracking-Verlust verkaufen."]
          },
          hostScope: {
        ...
        const evidenceFailures = {
          ...
          ga4: dedupe([
            ...(!overall ? ["Keine GA4-Seitenaufrufe für einen Vergleich vorhanden."] : []),
            ...(lastGa4Import.warning ? [lastGa4Import.warning] : []),
            ...(ga4InputProblem ? ["Der GA4-Export enthält Duplikate oder viele Seiten ohne Treffer in der Server-Datei."] : []),
            ...(hostReliability === "limited" ? ["Die Server-Datei enthält mehrere Websites oder Subdomains."] : []),
            ...(pageviewReliability === "limited" ? ["Die Server-Datei wurde nicht zuverlässig genug gelesen."] : []),
            ...(exportCompleteness.reliability === "limited" ? ["Der Server-Export ist nicht vollständig genug für einen belastbaren GA4-Vergleich."] : []),
        ...
        return {
          ...
          diagnostics: { recognitionRate, pageviewReliability, visitorReliability, ga4Reliability, conversionReliability, cacheRisk, chronologyIssue, botReliability, hostReliability, exportCompletenessReliability, trackingReliability: agg.trackingCapped ? "medium" : "high" },
      ...
      function qualityReason(data, metric, hasGa4) {
        if (metric === "views") {
          ...
          if (data.diagnostics.pageviewReliability === "high") return `Die Datei wurde sauber gelesen (${recognized}).`;
          if (data.diagnostics.pageviewReliability === "medium") return `${recognized} der Datei wurde gelesen. Einzelne Zeilen passen nicht zum erwarteten Format.`;
          return `Zu viele Zeilen passen nicht zum erwarteten Format (${recognized} gelesen).`;
        }
        if (metric === "visits") {
          ...
          if (data.diagnostics.visitorReliability === "high") return data.xffUsed ? "Echte Besucher-IPs aus dem Proxy-Feld wurden genutzt." : "Keine starke Proxy-Verzerrung sichtbar.";
          if (data.proxyKind) return "Proxy oder CDN erkannt. Besucher nur als grobe Spanne lesen.";
          if (data.xffMissing || data.xffPrivate) return "Das Proxy-Feld enthält keine brauchbaren Besucher-IPs.";
          return "Die Reihenfolge der Logs oder die Besucher-Erkennung macht diese Zahl ungenauer.";
        }
        if (metric === "purchases") {
          if (!data.hasSuccessUrl) return "Keine Kauf-/Danke-Seite angegeben.";
          if (data.diagnostics.conversionReliability === "high") return data.ga4Conversions === null ? "Kaufseite erkannt; doppelte Reloads werden reduziert." : "Kaufseite erkannt und mit GA4-Käufen verglichen.";
          return "Kaufseite erkannt, aber ohne Bestellnummer können Reloads die Zahl verzerren.";
        }
        if (metric === "ga4") {
        ...
        if (metric === "host") {
          ...
          return "Die Datei wirkt auf eine Website begrenzt.";
        }
        if (metric === "export") {
          if (!data.exportCompleteness || data.exportCompleteness.reliability === "high") return "Zeitraum, Lesbarkeit und Filterquote wirken plausibel.";
      ...
      function setPrecisionChecklist(data, hasGa4) {
        ...
        rows.push(item(!hasGa4 || !(data.ga4Import && data.ga4Import.warning), hasGa4
          ? ((data.ga4Import && data.ga4Import.warning) || "GA4-Zahlen wurden gelesen. Zeitraum und Seitenauswahl müssen gleich sein.")
          : "Keine GA4-Seitenzahlen eingetragen."));
      ...
      function render(data) {
        ...
        if (data.suspiciousClients > 0) {
          recognitionText += (recognitionText ? " " : "") + `${format(data.suspiciousClients)} auffällige Muster gefunden: sehr viele Aufrufe, aber kaum normale Seitenbestandteile. Das kann Bot-, Monitoring- oder Scraper-Traffic sein.`;
        }
        if (data.diagnostics.hostReliability === "limited") {
          recognitionText += (recognitionText ? " " : "") + `Die Datei enthält mehrere Websites oder Subdomains (${format(data.hosts.total)} erkannt). Setze oben einen Filter, damit ServerStory und GA4 wirklich dieselbe Website vergleichen.`;
        }
      ...
      function setVerdict(data, hasGa4, hasConv) {
        ...
        if (hasGa4) {
          ...
          if (cov < 85) {
            ...
            id("subline").textContent = `GA4 findet ${percent(cov)} der Aufrufe aus der Server-Datei. Eine kleine Lücke ist normal.${worstText}`;
            ...
            id("subline").textContent = `GA4 und die Server-Datei liegen nah beieinander (${percent(cov)}).`;
