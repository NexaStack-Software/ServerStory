      ...
      function qualityReason(data, metric, hasGa4) {
        if (metric === "views") {
          ...
          if (data.diagnostics.pageviewReliability === "medium") return `${recognized} der Datei wurde gelesen. Einzelne Einträge passen nicht zum erwarteten Format.`;
          return `Zu viele Einträge passen nicht zum erwarteten Format (${recognized} gelesen).`;
        }
        if (metric === "visits") {
          ...
          return "Die Reihenfolge der Einträge oder die Besucher-Erkennung macht diese Zahl ungenauer.";
        }
        if (metric === "purchases") {
          if (!data.hasSuccessUrl) return "Keine Kauf-/Danke-Seite angegeben.";
          if (data.diagnostics.conversionReliability === "high") return data.ga4Conversions === null ? "Kaufseite erkannt; doppelte Reloads werden reduziert." : "Kaufseite erkannt und mit Google-Analytics-Käufen verglichen.";
