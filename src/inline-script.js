...
198.51.100.77 - - [05/Jun/2026:12:10:00 +0200] "GET /agb HTTP/1.1" 200 800 "-" "-"`;
      ...
      function ga4UrlViews() {
        ...
        const lines = id("ga4-url-views").value.split(/\r?\n/).filter((line) => line.trim());
        let pathIndex = -1, metricIndex = -1, start = 0;
        if (lines.length) {
          const header = splitMetricLine(lines[0]).map((cell) => cell.toLowerCase());
          pathIndex = header.findIndex((cell) => /seite|page|path|pfad|url|landing/.test(cell));
          metricIndex = header.findIndex((cell) => /aufrufe|views|pageviews|screen page views|seitenaufrufe|ereignisanzahl/.test(cell));
          if (pathIndex >= 0 && metricIndex < 0 && header.some((cell) => /nutzer|users|active users|total users/.test(cell))) {
          ...
          if (pathIndex >= 0 && metricIndex >= 0) start = 1;
        }
      ...
      function makeAggregator(config) {
        ...
        function wildcardRe(pattern) {
          ...
          const escaped = normalized.replace(/[.+?^${}()|[\]\\]/g, "\\{{SCRIPT}}").replaceAll("*", ".*");
        ...
        function parseJsonLine(line) {
          ...
          try {
            ...
            const pt = parseIsoTime(pick(obj, ["time", "timestamp", "@timestamp", "ts", "EdgeStartTimestamp", "ClientRequestStartTime", "StartTime", "start", "reqTimeSec"]));
            if (!pt) return null;
            const request = pick(obj, ["ClientRequestURI", "request", "request_uri", "uri", "url", "path", "cs_uri_stem", "reqPath", "requestPath"]) || "/";
            const host = pick(obj, ["ClientRequestHost", "http_host", "host", "server_name", "cs_host", "reqHost"]);
            const kind = obj.ClientRequestURI || obj.EdgeResponseStatus ? "cloudflare"
              ...
              : obj.cp || obj.reqHost || obj.statusCode ? "akamai"
              : "json";
            return {
              kind,
              ip0: pick(obj, ["ClientIP", "ClientRequestIP", "remote_addr", "remoteAddress", "client_ip", "clientIp", "ip"]) || "-",
              pt,
              method: pick(obj, ["ClientRequestMethod", "method", "request_method", "cs_method", "reqMethod"]) || "GET",
              target: request,
              host,
              status: Number(pick(obj, ["EdgeResponseStatus", "status", "status_code", "response_status", "sc_status", "statusCode"])),
              ua: pick(obj, ["ClientRequestUserAgent", "user_agent", "http_user_agent", "ua", "cs_user_agent", "userAgent", "reqUserAgent"]),
              trailing: pick(obj, ["ClientRequestHeaderXForwardedFor", "x_forwarded_for", "http_x_forwarded_for", "forwarded_for"])
            };
          } catch (_) {
            return null;
          }
      ...
      function buildResult(agg, config) {
        ...
        const pageviewReliability = recognitionRate >= 0.95 && (agg.formatKind === "combined" || edgeKinds.has(agg.formatKind)) ? "high" : recognitionRate >= 0.8 ? "medium" : "limited";
        ...
        return {
          total: agg.total, parsed: agg.parsed, unrecognized: agg.unrecognized, meta: agg.meta || 0,
          ...
          proxyKind, hasChosen: chosen.length > 0, tableRows, overall,
